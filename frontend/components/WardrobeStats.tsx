...
export default function WardrobeStats({ userId, itemCount }: WardrobeStatsProps) {
  ...
  useEffect(() => {
    ...
    // Async wrapper avoids synchronous setState in effect body (lint rule)
    const run = async () => {
      setLoading(true);
      try {
        const result = await getWardrobeAnalytics(userId);
        setData(result);
  ...
  // Derive display data — hide stale stats when wardrobe is empty
  const displayData = itemCount === 0 ? null : data;
  ...
  if (!displayData || displayData.total === 0) return null;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
          ...
          <p className="text-2xl font-black">{displayData.total}</p>
          <p className="text-xs text-gray-500">pièces</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
          <Sparkles className="w-4 h-4 text-yellow-400 mx-auto mb-1" />
          <p className="text-2xl font-black">{displayData.estimated_outfit_count}</p>
          <p className="text-xs text-gray-500">tenues possibles</p>
        </div>
        {displayData.avg_look_score !== null && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
            <Star className="w-4 h-4 text-orange-400 mx-auto mb-1" />
            <p className="text-2xl font-black">{displayData.avg_look_score}<span className="text-sm text-gray-500">/5</span></p>
            <p className="text-xs text-gray-500">score moyen IA</p>
          </div>
        )}
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
          <Euro className="w-4 h-4 text-emerald-400 mx-auto mb-1" />
          <p className="text-2xl font-black">{displayData.wardrobe_value_eur.moyen}€</p>
      ...
      {displayData.colors.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {displayData.colors.slice(0, 6).map(c => (
      ...
      {/* Type breakdown */}
      {displayData.types.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Par type de vêtement</p>
          <div className="space-y-2">
            {displayData.types.slice(0, 5).map(t => {
              const pct = Math.round((t.count / displayData.total) * 100);
              return (
